...
def analyze_single_comparison(base_clauses, compare_text, base_name, compare_name, api_key, file_index):
    ...
    with tabs[tab_idx]:
        ...
        # 添加筛选功能 - 关键修复：添加唯一key参数
        min_similarity = st.slider(
            "最低相似度筛选", 
            0.0, 1.0, 0.0, 0.05,
            key=f"similarity_slider_{file_index}"  # 使用文件索引作为唯一标识
        )
        ...
        for i, (clause1, clause2, ratio) in enumerate(filtered_pairs):
            ...
            # 添加分析结果折叠框 - 添加唯一key参数
            with st.expander("查看合规性分析", expanded=False, key=f"analysis_expander_{file_index}_{i}"):
    ...
    if len(unmatched_base) > 0 and tab_idx < len(tabs):
        with tabs[tab_idx]:
            ...
            st.text_area("条款内容", unmatched_base[0] if unmatched_base else "", height=200,
                        key=f"unmatched_base_{file_index}")
    ...
    if len(unmatched_compare) > 0 and tab_idx < len(tabs):
        with tabs[tab_idx]:
            ...
            st.text_area("条款内容", unmatched_compare[0] if unmatched_compare else "", height=200,
                        key=f"unmatched_compare_{file_index}")
